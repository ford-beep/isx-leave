...
export interface CalendarProps {
   ...
   *
   * Used by Admin Calendar and other existing calendar views.
   */
  requests: LeaveRequest[];

  /**
   * Privacy-safe company-wide approved leave.
   *
   * Used by the employee Dashboard so everyone can see who is
   * on approved leave without exposing the full leave request.
   */
  companyLeaves?: CompanyCalendarLeave[];

  birthdays: CalendarBirthday[];

  /** Admin shows employee names. Employee mode is the personal/company dashboard. */
  mode: "employee" | "admin";

  /** Route the month arrows point at. */
  basePath: string;

  /** Extra query params preserved when changing month. */
...
export function MonthCalendar({
   ...
   *
   * Admin Calendar:
   *   uses normal LeaveRequest data.
   *
   * Employee Dashboard:
   *   uses companyLeaves so employees can see approved leave
   *   across the company without receiving private request data.
   */
  ...
  for (const r of requests) {
    for (let d = r.startDate; d <= r.endDate; ) {
      ...
      list.push({
        ...
        title:
          mode === "admin"
            ? `${r.employeeName ?? "Employee"} — ${r.leaveTypeLabel} — ${r.status}`
      ...
      <div className="cal">
        ...
        <div className="cal-grid">
          {cells.map((date) => {
            ...
            const workMode =
              explicitWorkMode ?? defaultWorkMode;

            const isOffice =
              isWorkingDay && workMode === "office";

            const isWFH =
              isWorkingDay && workMode === "wfh";
