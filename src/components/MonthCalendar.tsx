...
import { companyToday, monthGrid, monthLabel, parseISO, WEEKDAY_SHORT } from "@/lib/date";
import type {
  Holiday,
  LeaveRequest,
  WorkScheduleDay,
...
function qs(base: string, y: number, m: number, extra?: Record<string, string>) {
  const p = new URLSearchParams({ y: String(y), m: String(m), ...(extra ?? {}) });
...
export function MonthCalendar({
  ...
  const workModeByDate = new Map(
  workSchedule.map((d) => [d.date, d.mode]),
);
  const holidayByDate = new Map(holidays.filter((h) => h.active).map((h) => [h.date, h]));
  

  // Expand each request into the individual dates it covers.
  const entriesByDate = new Map<string, Array<{ label: string; status: string; title: string }>>();
  for (const r of requests) {
    for (let d = r.startDate; d <= r.endDate; ) {
      ...
      const nd = parseISO(d); nd.setUTCDate(nd.getUTCDate() + 1);
    ...
    <div>
      <div className="cal-head">
        ...
        <div className="row" style={{ marginLeft: "auto", gap: 6 }}>
          <Link className="btn btn-sm" href={qs(basePath, prev.y, prev.m, extraQuery)} aria-label="Previous month">
            <IconChevronLeft size={15} />
          </Link>
          <Link className="btn btn-sm" href={qs(basePath, Number(today.slice(0, 4)), Number(today.slice(5, 7)), extraQuery)}>
            Today
          </Link>
          <Link className="btn btn-sm" href={qs(basePath, next.y, next.m, extraQuery)} aria-label="Next month">
        ...
        <div className="cal-tag wfh" title="Work from home">
          WFH
                ...
                {entries.slice(0, 2).map((e, i) => (
                  <div key={i} className={`cal-tag ${e.status}`} title={e.title}>{e.label}</div>
                ))}
                {entries.length > 2 && <div className="cal-tag more">+{entries.length - 2} more</div>}
              </div>
            );
          })}
        </div>
