...
const ACTION_LABEL: Record<string, string> = {
  ...
  "comp_day.credit_granted": "granted a Comp Day",
  "comp_day.credit_updated": "updated a Comp Day",
  "comp_day.credit_removed": "removed a Comp Day",
...
function summarise(
  ...
  if (action.startsWith("comp_day.")) {
    return [
      m.employee_name || m.employee_id,
      m.earned_date && `Worked ${m.earned_date}`,
      m.earned_year && `Year ${m.earned_year}`,
      m.note && `“${m.note}”`,
    ]
      .filter(Boolean)
      .join(" · ");
  ...
  // ---------------------------------------------------------
  // Per-date work schedule override
  // ---------------------------------------------------------
  ...
  // ---------------------------------------------------------
  // Generic before / after audit entries
  // ---------------------------------------------------------

  if (m.after && typeof m.after === "object") {
    const a = m.after as Record<
      string,
      unknown
    >;
    ...
    <>
      <div className="page-head">
        <div className="grow">
          ...
          <p className="muted">
            Append-only record of every decision and
            configuration change. Written by the database
            itself, so it can&apos;t be bypassed or edited
      ...
      <Card>
        <CardHead
          ...
          sub={`${entries.length} most recent entries`}
        />
