...
-- ============================================================================
-- EMPLOYEE SELF-CANCEL STATUS GUARD
-- Overrides the admin-only guard introduced in migration 0006.
-- ============================================================================
...
begin
  if new.status is distinct from old.status
    ...
    -- Employees may only cancel their own Annual / Comp Day leave
    -- while it is Pending or Approved, and only before the leave starts.
    if not (
      old.employee_id = app.current_user_id()
      and old.status in ('pending', 'approved')
      and new.status = 'cancelled'
      and old.leave_type in ('annual', 'comp_day')
      and (current_timestamp at time zone 'Asia/Bangkok')::date < old.start_date
    ) then
...
-- ============================================================================
-- EMPLOYEE SELF-CANCEL RPC
-- Narrow entry point for employees to cancel their own leave.
-- ============================================================================

create or replace function app.cancel_own_leave(
  p_request_id uuid
)
returns table (
...
begin
  ...
  if caller is null or not app.is_active_user() then
  ...
  if req.leave_type not in ('annual', 'comp_day') then
    raise exception 'LEAVE_SELF_CANCEL_NOT_ALLOWED';
  end if;
  ...
  if (current_timestamp at time zone 'Asia/Bangkok')::date >= req.start_date then
    raise exception 'LEAVE_SELF_CANCEL_TOO_LATE';
  end if;

  previous_status := req.status::text;
  leave_type := req.leave_type::text;
...
revoke all
on function app.cancel_own_leave(uuid)
from public;
