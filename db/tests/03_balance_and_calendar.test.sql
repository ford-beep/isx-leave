...
-- ---------------------------------------------------------------------------
select pg_temp.eq((app.calc_leave_days('2026-09-18', '2026-09-22') ->> 'totalCalendarDays')::int, 5,
  'worked example: 5 calendar days');
select pg_temp.eq((app.calc_leave_days('2026-09-18', '2026-09-22') ->> 'excludedNonOfficeDays')::int, 2,
  'worked example: 2 non-working days excluded (Sat, Sun)');
select pg_temp.eq((app.calc_leave_days('2026-09-18', '2026-09-22') ->> 'excludedHolidays')::int, 0,
  'worked example: no holidays in range');
select pg_temp.eq((app.calc_leave_days('2026-09-18', '2026-09-22') ->> 'leaveDays')::int, 3,
...
values (:'mike'::uuid, 'annual', '2026-11-16', '2026-12-08', 'Long break');

select pg_temp.eq(
  (select leave_days from leave_requests
...
update leave_entitlements set total_days = 15 where employee_id = :'mike'::uuid and year = 2026;
select pg_temp.raises(
  'update leave_requests set status = ''approved''
   where employee_id = ''44444444-4444-4444-8444-444444444444'' and reason = ''Long break''',
...
-- Yearly isolation (§23)
-- ---------------------------------------------------------------------------
select pg_temp.eq((select entitlement from app.leave_balance(:'jane'::uuid, 2027)), 15::numeric,
  '2027 carries its own entitlement row');
select pg_temp.eq((select approved from app.leave_balance(:'jane'::uuid, 2027)), 0::numeric,
...
select pg_temp.ok(
  (select count(*) from audit_logs where entity_type = 'office_day'
...
select pg_temp.eq(
  (select start_date from leave_requests
    where employee_id = :'jane'::uuid and status = 'approved' and end_date >= date '2026-08-14'
    order by start_date limit 1),
  date '2026-09-18', 'Jane''s next upcoming approved leave is 18 Sep 2026');
